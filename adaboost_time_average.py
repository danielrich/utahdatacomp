...
def run_average_time_model( model, model_name, X, Y):
    ...
    ytotal = 0
    for i in range(1,10):
	    xsplit = numpy.split(X, i)[-1]
	    ysplit = numpy.split(Y, i)[-1]
...
run_model(ensemble.AdaBoostRegressor(loss="exponential"), " average of 10 segmented exponential loss Ada Boost regressor ", X, Y)
